...
public class ClientSagaConsumer {
    ...
    private final ConcurrentHashMap<String, CompletableFuture<ClientCreatedResponseDto>> createClientResponses;

    public ClientSagaConsumer(ConcurrentHashMap<String, CompletableFuture<ClientCreatedResponseDto>> createClientResponses) {
        this.createClientResponses = createClientResponses;
    }

    @RabbitListener(queues = RabbitConstants.CREATE_CLIENT_RESP_QUEUE)
    public void onClientCreated(ClientCreatedResponseDto evt) {
