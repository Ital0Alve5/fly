  ...
  <div class="min-h-screen">
    <nav class="p-4 shadow-md mt-8">
      <div class="container mx-auto flex justify-between items-center">
        <h1 class="text-white text-2xl font-semibold">Bem-vindo, Fulano</h1>
        <span class="text-white text-lg">Saldo de Milhas: <strong>{{ saldoMilhas }}</strong></span>
      </div>
            ...
            <TableBody>
              <TableRow v-for="reserva in reservas" :key="reserva.id">
                ...
                <TableCell class="text-center">
                  <Button v-if="reserva.status === 'Reservado'" class="mr-2" @click="verReserva(reserva.id)">Ver Reserva</Button>
                  <Button v-if="reserva.status === 'Reservado'" variant="destructive" @click="cancelarReserva(reserva.id)">Cancelar</Button>
                </TableCell>
              </TableRow>
            </TableBody>
          </Table>
